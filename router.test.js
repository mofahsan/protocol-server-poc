...
router.post("/extractPayload", async (req, res) => {
  ...
  if(!await validateSchema(payload,session.schema[config])){
    return res.status(400).send("schema validation failed")
  }

  const {payload: becknPayload, session: updatedSession} = extractBusinessData(
    session,
