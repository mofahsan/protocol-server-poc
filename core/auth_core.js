const LOOKUP_URI = process.env.ondc_LOOKUP_URI, PRIVATE_KEY = process.env.PRIVATE_KEY, SUBSCRIBER_ID = process.env.subscriber_id, UNIQUE_KEY = process.env.UNIQUE_KEY
...
async function generateHeader(message){
    const result =  await createAuthorizationHeader({
       ...
       bapId: SUBSCRIBER_ID , // Subscriber ID that you get after registering to ONDC Network
